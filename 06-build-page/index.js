...
(async () => {
    try {
        const dirCreation = await mkdir(path.join(__dirname, 'project-dist'), { recursive: true });
        ...
        readStream.on('end', async () => {
            ...
            for (let item of components) {
                const componentFilePath = path.join(path.join(__dirname, 'components'), `${item[1]}.html`); 
...
async function bundleCss() {
    ...
    files.forEach(async item => {
      ...
      if (ext === '.css') {
        ...
        let data = '';
        readStream.on('data', chunk => data += chunk);
        readStream.on('end', () => fs.appendFile(
          path.join(__dirname, 'project-dist', 'style.css'), data,
          (err) => {
              if (err) throw err;
          }
      ));
